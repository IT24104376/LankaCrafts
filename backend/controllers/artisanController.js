...
exports.getArtisans = async (req, res, next) => {
  try {
    ...
    const artisans = await Artisan.find(filter).sort({ submittedDate: -1 });
...
exports.updateArtisanStatus = async (req, res, next) => {
  try {
    const { status } = req.body;
    if (!['verified', 'rejected', 'pending'].includes(status)) {
      return res.status(400).json({ success: false, message: 'Invalid status. Must be verified, rejected, or pending.' });
    }
    const artisan = await Artisan.findByIdAndUpdate(req.params.id, { status }, { new: true, runValidators: true });
    if (!artisan) return res.status(404).json({ success: false, message: 'Artisan not found.' });
