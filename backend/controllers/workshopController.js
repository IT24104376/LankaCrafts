...
exports.getWorkshops = async (req, res, next) => {
  try {
    ...
    const workshops = await Workshop.find(filter).sort({ submittedDate: -1 });
...
exports.updateWorkshopStatus = async (req, res, next) => {
  try {
    ...
    const workshop = await Workshop.findByIdAndUpdate(req.params.id, { status }, { new: true, runValidators: true });
    if (!workshop) return res.status(404).json({ success: false, message: 'Workshop not found.' });
