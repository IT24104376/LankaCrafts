...
export function TouristLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await authApi.loginWithEmail(email, password, 'tourist');
      session.set(user);
        ...
        <motion.div
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.button
              ...
              className="w-full py-3.5 rounded-xl text-white font-semibold text-sm font-body transition-colors mt-2 disabled:opacity-60"
              style={{
