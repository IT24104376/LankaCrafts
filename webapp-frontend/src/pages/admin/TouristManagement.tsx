...
import { getTourists, toggleTouristStatus as apiToggleTouristStatus } from '../../api/adminApi';
...
export function TouristManagement() {
  ...
  const fetchTourists = async () => {
    ...
    try {
      ...
      setError('Failed to load tourists. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleStatus = async (id: string) => {
    try {
      const res = await apiToggleTouristStatus(id);
      const updated = res.data.data;
      setTourists(prev => prev.map(t => t._id === id ? { ...t, status: updated.status } : t));
      if (selectedTourist?._id === id) {
        setSelectedTourist(prev => prev ? { ...prev, status: updated.status } : null);
      }
    } catch {}
  };
    ...
    <div className="flex flex-col h-full">
      ...
      {loading && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="font-medium text-sm">Loading tourists...</p>
        </div>
      )}
      {error && !loading && (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="text-left px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Tourist</th>
                  <th className="text-left px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Country</th>
                  <th className="text-left px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Workshops</th>
                  ...
                  <motion.tr
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{ backgroundColor: tourist.color }}>
                          {tourist.initials}
                        </div>
                        <div>
                          <p className="font-semibold text-gray-900 text-sm">{tourist.name}</p>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm font-semibold text-gray-900">{tourist.totalBookings}</span>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5 text-sm text-gray-500">
                        ...
                        {tourist.joinedDate ? new Date(tourist.joinedDate).toLocaleDateString() : '—'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${tourist.status === 'active' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => handleToggleStatus(tourist._id)}
                          ...
                          title={tourist.status === 'active' ? 'Suspend' : 'Activate'}>
          ...
          <>
            ...
            <motion.div
              ...
              <div
                ...
                style={{ backgroundColor: selectedTourist.color }}>

                <div className="absolute inset-0 opacity-15">
                  <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                      <pattern id="drawer-pattern" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
                ...
                <div className="px-6 pb-6">
                  ...
                  <div className="flex items-end justify-between -mt-8 mb-4">
                    ...
                    <span
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full ${selectedTourist.status === 'active' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                  ...
                  <p className="text-gray-500 text-sm mb-5 flex items-center gap-1">
                    <GlobeIcon className="w-3.5 h-3.5" /> {selectedTourist.country || '—'}
                  </p>
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-800">{selectedTourist.email}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-800">{selectedTourist.phone || '—'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-800">
                          {selectedTourist.joinedDate ? new Date(selectedTourist.joinedDate).toLocaleDateString() : '—'}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <h3 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                      ...
                      Workshop Attendance ({selectedTourist.workshopsAttended?.length || 0})
                    </h3>
                    {selectedTourist.workshopsAttended?.length > 0 ?
                      <div className="space-y-2">
                        {selectedTourist.workshopsAttended.map((ws, idx) =>
                          <div
                            key={ws._id || idx}
                            className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl">
                            <div
                              className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0"
                              style={{ backgroundColor: CRAFT_COLORS[ws.craft] || '#2F5D50' }}>
                              {ws.craft ? ws.craft.charAt(0) : 'W'}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold text-gray-900 truncate">{ws.workshop}</p>
                              <p className="text-xs text-gray-400">
                                with {ws.artisan} · {ws.date ? new Date(ws.date).toLocaleDateString() : ''}
                              </p>
                            </div>
                          </div>
                        )}
                      ...
                      <div className="text-center py-6 bg-gray-50 rounded-xl">
                        ...
                        <p className="text-sm text-gray-400">No workshops attended yet</p>
                      </div>
                    }
                  </div>
                </div>
              ...
              <div className="p-6 border-t border-gray-100 shrink-0">
                <button
                  onClick={() => handleToggleStatus(selectedTourist._id)}
